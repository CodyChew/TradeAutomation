...
class LPFSNewMT5AccountValidationTests(unittest.TestCase):
    ...
    def test_icmarkets_production_config_example_has_separate_runtime_identity(self) -> None:
        ...
        self.assertEqual(config["account_profile"]["intended_live_scale"], 1.0)
        ...
        self.assertEqual(config["dry_run"]["risk_bucket_scale"], 2.0)
        self.assertEqual(config["dry_run"]["max_risk_pct_per_trade"], 1.5)
        self.assertEqual(config["dry_run"]["max_open_risk_pct"], 12.0)
        self.assertEqual(config["live_send"]["risk_bucket_scale"], 1.0)
        self.assertEqual(config["live_send"]["max_risk_pct_per_trade"], 0.75)
        self.assertEqual(config["live_send"]["max_open_risk_pct"], 6.0)
        self.assertEqual(config["live_send"]["execution_mode"], "DRY_RUN")
        self.assertFalse(config["live_send"]["live_send_enabled"])
