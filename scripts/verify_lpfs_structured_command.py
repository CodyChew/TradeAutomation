...
STRICT_MT5_STEP_CONTRACT_VERSION = 4
...
STRICT_MT5_EXECUTION_SCHEMA_VERSION = 1
...
PINNED_SAFETY_PROFILE_DOCUMENT_SHA256S = {
    ...
    "a1a4f09a5ff6f1698b0c1390347d44da16f52b646cd79545bbea9db6afb3af16",
    "37f2491db28a2422c8b9e473ed134e7bcc82afb8334dd2652bea7e8a7de885bd",
    "3089fc133c7949f75879fadf4a7c5f1b9e65e7cefc5f3087c287c7017f399d1a",
}
...
def verify_strict_mt5_bundle(
    ...
    *,
    ...
    expected_hashes = {
        "command": expected_command_sha256,
        "strict_mt5_script": expected_strict_mt5_script_sha256,
    }
    for label, expected_hash in expected_hashes.items():
        if not isinstance(expected_hash, str) or not SHA256_RE.fullmatch(expected_hash):
            failures.append(f"expected {label} SHA-256 must be valid lowercase hexadecimal")
    ...
    if not required_fields:
        ...
        failures.append(f"step expectation set mismatch: missing={missing!r} undeclared={undeclared!r}")
    if declared_artifacts is None:
        failures.append("manifest declarations are required for strict-MT5 verification")
    ...
    command_text = texts.get("command", "")
    script_text = texts.get("strict_mt5_script", "")
    stdout_text = texts.get("stdout", "")
    stderr_text = texts.get("stderr", "")
    if not command_text.strip():
        failures.append("command artifact is empty")
    else:
        if len(command_text) >= STRICT_MT5_COMMAND_SAFE_LENGTH:
            failures.append(
                f"strict-MT5 command length {len(command_text)} exceeds safe threshold "
                f"{STRICT_MT5_COMMAND_SAFE_LENGTH}"
            )
        if "import base64;exec" in command_text or " -c " in command_text:
            failures.append("strict-MT5 command must not use inline python -c execution")
        if script_text.strip() and script_text.strip() in command_text:
            failures.append("strict-MT5 command must not inline the full strict script")
        if script_text.strip():
            import base64

            if base64.b64encode(script_text.encode("utf-8")).decode("ascii") in command_text:
                failures.append("strict-MT5 command must not inline the strict script as base64")
    if not script_text.strip():
        failures.append("strict MT5 script artifact is empty")
    if not stdout_text.strip():
        failures.append("strict-MT5 stdout artifact is empty")
    verification_marker = f"LPFS_STRICT_MT5_SCRIPT_SHA256_VERIFIED={expected_strict_mt5_script_sha256}"
    verification_marker_count = sum(
        1 for line in stdout_text.splitlines() if line.strip() == verification_marker
    )
    if verification_marker_count != 1:
        failures.append(
            "strict-MT5 stdout must contain exactly one script verification marker; "
            f"found {verification_marker_count}"
        )
    if stderr_text.strip():
        failures.append("stderr artifact is not empty")
    ...
    if timeout_value not in {"true", "false"}:
        ...
        failures.append("strict-MT5 command timed out")

    nonempty_stdout = [line.strip() for line in stdout_text.splitlines() if line.strip()]
    marker_lines = [line for line in nonempty_stdout if marker_valid and line.startswith(marker)]
    if stdout_text and len(nonempty_stdout) != 2:
        failures.append(f"stdout must contain exactly two nonempty strict-MT5 lines; found {len(nonempty_stdout)}")
    if len(marker_lines) != 1:
        failures.append(f"stdout must contain exactly one {marker!r} line; found {len(marker_lines)}")
    ...
    if "execution" in texts:
        try:
            decoded_execution = _strict_json_loads(texts["execution"])
            if not isinstance(decoded_execution, dict):
                failures.append("strict-MT5 execution artifact must contain a JSON object")
            else:
                execution = decoded_execution
                _require_exact_keys(
                    execution,
                    {
                        ...
                        "exit_code",
                        "expected_command_sha256",
                        "expected_strict_mt5_script_sha256",
                        "remote_strict_mt5_script_sha256_verified",
                        "schema_version",
                        "stderr_empty",
                        "stdout_nonempty",
                        "strict_mt5_script_hash_matches_expected",
                        "strict_mt5_script_sha256",
                        "strict_mt5_script_source",
                        "timed_out",
                        "timeout_seconds",
                    },
                    "strict-MT5 execution artifact",
                )
        except Exception as exc:
            failures.append(f"strict-MT5 execution artifact is invalid JSON: {type(exc).__name__}: {exc}")
    ...
    expected_execution = {
        ...
        "exit_code": 0,
        "expected_command_sha256": expected_command_sha256,
        "expected_strict_mt5_script_sha256": expected_strict_mt5_script_sha256,
        "remote_strict_mt5_script_sha256_verified": True,
        "schema_version": STRICT_MT5_EXECUTION_SCHEMA_VERSION,
        "stderr_empty": True,
        "stdout_nonempty": True,
        "strict_mt5_script_hash_matches_expected": True,
        "strict_mt5_script_sha256": expected_strict_mt5_script_sha256,
        "strict_mt5_script_source": STRICT_MT5_SOURCE,
        "timed_out": False,
    }
    if execution is not None:
        for field, expected in expected_execution.items():
            ...
            if type(actual) is not type(expected) or actual != expected:
                failures.append(
                    f"strict-MT5 execution mismatch for {field!r}: "
                    f"expected={expected!r} actual={actual!r}"
                )
        timeout_seconds = execution.get("timeout_seconds")
        if isinstance(timeout_seconds, bool) or not isinstance(timeout_seconds, int) or timeout_seconds <= 0:
            failures.append("strict-MT5 execution timeout_seconds must be a positive integer")
    ...
    return {
        ...
        "status": "PASS" if not failures else "STOPPED",
        "reason": (
            "manifest-bound strict-MT5 command, script, execution metadata, and expectations verified"
            if not failures
            else "; ".join(failures)
        ),
        "step": step_name,
...
def load_safety_profile(
    ...
    *,
    ...
    for step_name in required_steps:
        ...
        if contract_version == STRUCTURED_STEP_CONTRACT_VERSION:
        ...
        elif contract_version == STRICT_MT5_STEP_CONTRACT_VERSION:
            _require_exact_keys(
                step,
                {
                    "bundle_kind",
            ...
            if step.get("bundle_kind") != STRICT_MT5_BUNDLE_KIND:
                raise ValueError(
                    f"unsupported safety profile step {step_name!r} bundle_kind: {step.get('bundle_kind')!r}"
                )
            expected_command_sha256 = step.get("expected_command_sha256")
            expected_script_sha256 = step.get("expected_strict_mt5_script_sha256")
            if not isinstance(expected_command_sha256, str) or not SHA256_RE.fullmatch(expected_command_sha256):
                raise ValueError(f"safety profile step {step_name!r} expected_command_sha256 must be valid")
            if not isinstance(expected_script_sha256, str) or not SHA256_RE.fullmatch(expected_script_sha256):
                raise ValueError(
                    f"safety profile step {step_name!r} expected_strict_mt5_script_sha256 must be valid"
                )
            marker = step.get("marker")
            if not isinstance(marker, str) or not marker:
                raise ValueError(f"safety profile step {step_name!r} marker must be a nonempty string")
            ...
            validated_steps[step_name] = {
                ...
                "bundle_kind": STRICT_MT5_BUNDLE_KIND,
                "expected_command_sha256": expected_command_sha256,
                "expected_strict_mt5_script_sha256": expected_script_sha256,
                "marker": marker,
                "required_expectation_fields": required_fields,
                "expectations": expectations,
            }
    ...
    *,
    ...
    for step_name, spec in profile_steps.items():
        try:
            ...
            elif spec["contract_version"] == STRICT_MT5_STEP_CONTRACT_VERSION:
                result = verify_strict_mt5_bundle(
                    root,
                    step_name,
                    spec["marker"],
                    expectations=spec["expectations"],
                    required_expectation_fields=spec["required_expectation_fields"],
                    expected_command_sha256=spec["expected_command_sha256"],
                    expected_strict_mt5_script_sha256=spec["expected_strict_mt5_script_sha256"],
                    declared_artifacts=manifest["declared_artifacts"],
                )
            else:
